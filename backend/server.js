...
app.post('/api/moderator/create-contest', verifyToken, async (req, res) => {
    try {
        ...
        const totalScore = problems.reduce((sum, p) => sum + (parseInt(p.score) || 20), 0);
...
// POST /api/moderator/process-document
app.post('/api/moderator/process-document', verifyToken, async (req, res) => {
    try {
        const { documentContent, language, difficulty } = req.body;
        ...
        const chatCompletion = await groq.chat.completions.create({
            messages: [
                {
                    role: "system",
                    content: "Extract programming problems. Return JSON with 'problems' array. Each problem must have 2 sample_inputs/outputs and 5 hidden_inputs/outputs."
                },
                {
                    role: "user",
                    content: `Extract from: ${documentContent.substring(0, 6000)}`
                }
            ],
            model: "llama-3.3-70b-versatile",
            response_format: { type: "json_object" }
        ...
        res.json({ success: true, data: JSON.parse(chatCompletion.choices[0].message.content) });
    } catch (err) {
...
async function updateRegularStudentResults(studentEmail, contestId, contest, submission) {
    try {
        ...
        // Fetch existing results to update specific problem scores
        const { Item: existingItem } = await client.send(new GetItemCommand({
            TableName: "StudentResults",
            Key: ddbMarshall({ result_id: resultId })
        }));
        ...
        // Calculate Totals and Max Score
        ...
        let status = problemsSolved === totalProblems ? 'completed' : 'in_progress';

        // Update the Database with the new max_score field
        await client.send(new UpdateItemCommand({
            TableName: "StudentResults",
            Key: ddbMarshall({ result_id: resultId }),
            UpdateExpression: "SET problem_scores = :ps, total_score = :ts, max_score = :ms, problems_solved = :psv, #st = :st, updated_at = :ua, contest_name = :cn, contest_id = :cid, student_email = :se",
...
app.post('/api/student/submit-solution', verifyToken, async (req, res) => {
    try {
        ...
        if (alreadyPassed || currentAttempts >= 2) {
            return res.status(403).json({ 
                success: false, 
                message: "This question is locked. You cannot submit again." 
            });
        }
...
async function runOnCompiler(language, code, stdin) {
    try {
        ...
        return {
            ...
            error: String(result.stderr || result.error || "")
        };
    } catch (e) {
        return { output: "", error: "Execution Timeout/Error" };
    }
}
...
// New public route for registration dropdown (No verifyToken middleware)
// Ensure this is outside of any other route handlers
// Public route for student registration dropdown
app.get('/api/public/colleges', async (req, res) => {
    try {
        ...
        console.error("Public College Fetch Error:", err);
        res.status(500).json({ success: false, message: "Error fetching colleges" }); 
    }
});
...
// Optimized access check in server.js
...
app.get('/api/moderator/contest/:id/export-csv', verifyToken, async (req, res) => {
    try {
        const contestId = req.params.id;
        const { Items } = await client.send(new ScanCommand({
            TableName: "StudentResults",
